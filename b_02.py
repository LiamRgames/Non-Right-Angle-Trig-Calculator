...
while True:
    ...
    if goal == "area":
        ...
        triangle_area = 0.5 * area_variables["a"] * area_variables["b"] * math.sin(math.radians(area_variables["C"]))
        ...
        if method == "sine":
            ...
            side_length = graph_side_lengths["b"] = sine_side_lengths["a"]/math.sin(math.radians(sine_side_lengths["A"])) * math.sin(math.radians(sine_side_lengths["B"]))
            ...
            if cosine_side_lengths["A"] >= 180:
                print("That angle is too large")
                continue
            side_length = graph_side_lengths["a"] = np.sqrt(cosine_side_lengths["b"]**2 + cosine_side_lengths["c"]**2 - 2 * cosine_side_lengths["b"] * cosine_side_lengths["c"] * math.cos(math.radians(cosine_side_lengths["A"])))
        ...
        if method == "sine":
            ...
            if sine_angle_values["B"] >= 180:
                ...
                angle = graph_angles["A"] = math.degrees(math.asin(sin_check))
            else:
            ...
            if -1 <= cos_check <= 1:
                angle = graph_angles["A"] = math.degrees(math.acos(cos_check))
            else:
